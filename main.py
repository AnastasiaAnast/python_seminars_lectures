...
# n = 5 #тип данных int
# n = 1.89  #тип данных float
# n = None #присвоить пустое значение переменной - потребуется позже, но еще не ясно, какой тип данных будет иметь
# print(type(n)) #вывести тип данных переменной n - ф-я type
...
'''
Кортеж - tuple () - неизменяемый список. Защищает данные от изменений. Занимает МЕНЬШЕ места в ПАМЯТИ, работает БЫСТРЕЕ списков.
'''
...
'''
Словарь - dictionary {} - неупорядоченная коллекция произвольных объектов с доступом по КЛЮЧУ (строка/число).
'''
# d = {} # создали словарь
