...
- эквиваленция/равнозначность (↔) "тогда и только тогда, когда/необходимо и достаточно" - логич операция, ставящая в соответствие двум высказываниям новое, являющееся истинным, когда оба исходных высказывания истинны или оба исходных высказывания ложны.
А ↔ В = А ⊕ В (черточка над всем выражением А ⊕ В)
А ↔ В = Ā * B(черточка сверху) + A * B
А       В       А ↔ В
0       0       1
0       1       0
1       0       0
...
# if condition1: # будет проверяться это условие
#     operator # если оно оказалось верным, будет выполняться этот оператор; дальше проверок не будет
# elif condition2: # Если первое условие оказалось неверным - будут проверяться последующие условия по порядку до последнего.
#     operator
# elif condition3:
#     operator
# else:         # Если все условия оказались неверными, то выполняется блок кода, который находится в else
#     operator
...
# flag = True
# i = 2
# while flag:        #равняется записи    while flag == true:
#     if n % i == 0: 
#         flag = False
#         print(i)
#     elif i > n // 2:
...
'''
ф-я range генерирует последовательность, принимает 3 аргумента: откуда начинаем (по умолчанию = 0), на каком числе заканчиваем, шаг (по умолчанию шаг = 1). Хорошо использовать совместно с for.
'''

'''
если в скобках указано только одно значение (аргумент) - будет генерироваться последовательность от 0 до указанного знач-я НЕ включая его
'''
# r = range(5) # 0 1 2 3 4

'''
если в скобках указаны два значения (аргумента) - с первого (включая его) - будет генерироваться последовательность до указанного 2-го знач-я НЕ включая его
'''
# r = range(2, 5) # 2 3 4
# r = range(0, -5) #ничего не будет выводиться, тк шаг по умолчанию 1, от 0 до -5 мы не сможем дойти, тк будет постоянно прибавляться 1.
# r = range(1, 10, 2) # 1 3 5 7 9
# r = range(100, 0, -20) # 100 80 60 40 20  #если нужно идти с отрицательным шагом - обязательно надо указывать ТРИ АРГУМЕНТА, последний из которых (шаг) будет отрицательным значением.
...
''' 
Цикл for также можно использовать для СТРОК, у них также есть нумерация, и как и у массивов она начинается с 0.
...
# print(text[:2]) # вывести элементы с 0 индекса по 2-й индекс не включая его - похоже на ф-ю range. Также может идти отрицательная индексация.
